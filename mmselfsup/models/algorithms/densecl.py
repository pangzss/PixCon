...
class DenseCL(BaseModel):
    ...
    def calc_loss(self, im_q, im_k):
        ...
        if self.asym:
            ...
            q_grid = rearrange(q_grid, "(b m) c -> b c m", b=im_q.shape[0])
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k, _ = self.encoder_k[1](k_b, "image")
        ...
        # feat point set sim
        backbone_sim_matrix = torch.matmul(q_b.permute(0, 2, 1).detach(), k_b)
        densecl_sim_ind = backbone_sim_matrix.argmax(dim=2)  # NxS^2
        ...
        return loss_single, loss_dense, k, k2

    def forward_train(self, img, masks=None, **kwargs):
        ...
        loss_single_12, loss_dense_12, k_single_12, k_dense_12 = self.calc_loss(im_q, im_k)
        losses["loss_single"] = loss_single_12
        losses["loss_dense"] = loss_dense_12
        k_single = k_single_12
        k_dense = k_dense_12
        if self.sym_loss:
            loss_single_21, loss_dense_21, k_single_21, k_dense_21 = self.calc_loss(im_k, im_q) 
            losses["loss_single"] += loss_single_21
            losses["loss_dense"] += loss_dense_21
            if np.random.rand() < 0.5:
                k_single = k_single_21
                k_dense = k_dense_21
            
        self._dequeue_and_enqueue(k_single)
